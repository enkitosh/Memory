...
initialize_table(int op[], int dim_i, int dim_j)
{
	...
	for (i = 0; i < new_table->row_dimension; i++) {
		for (j = 0; j < new_table->col_dimension; j++) {
			...
			new_table->info[i][j].cell_width = 0;
		}
	}
...
print(table_t * table)
{
	...
	*/
	...
	if(table->options[3] == ENUMERATE || table -> options[3] == ENUMERATE_REVERSED) isEnumerated = 1;
	...
	if (isEnumerated) {
        if(table->options[3] != ENUMERATE_REVERSED) {
		for (i = 0; i < table->col_dimension; i++) {
			if (align == LEFT) {
				if (i == 0) {
					ms(1, ' ');
            ...
            /*Do this if table is enumerated in reverse order*/ 
            	for (i = table->col_dimension - 1; i >= 0; i--) {
			if (align == LEFT) {
				if (i == table->col_dimension -1) {
					ms(1, ' ');
